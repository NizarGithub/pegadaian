...
switch ($page) {
	...
	case 'simpan_order':
		...
		case 'reset':
			...
			delete_config("item_keterangan_details_tmp", $where_purchase_id_);
			header("Location: purchase.php?page=form");
			break;

		case 'close':
			$purchase_id = get_isset($_GET['purchase_id']);
			$q_close = mysql_query("select * from purchases_details_tmp where purchase_id = ".$purchase_id);
			while($r_close = mysql_fetch_array($q_close)){
		...
		case 'save_payment':
			...
			$uang_muka 					= $_POST['output'];
			$uang_sisa 					= $_POST['output_sisa'];
			$i_tgl 							= $_POST['i_tgl'];
			$i_tgl 							= format_back_date3($i_tgl);
			$i_bank_id  				= $_POST['i_bank_id'];
			...
			if($i_payment_method == 5){
				...
				$data_hutang ="'',
											...
											''
				";
			create_hutang($data_hutang);
			$check_supplier_id = check_supplier_id($i_supplier);
			$data_hutang_tot = "'',
											'".$i_supplier."',
											'".$r_purchases['purchases_date']."',
											'".$uang_sisa."'
			";
			if($check_supplier_id==0){
				create_config("hutang1", $data_hutang_tot);
			} else {
				update_supplier_id($uang_sisa, $r_purchases['supplier_id']);
			}
			} else {
				$lunas = 0;
			}
			...
			while ($r_purchases_details = mysql_fetch_array($q_purchases_details)) {
				$data_detail = "'',
											 ...
											 ";
			create_purchase_details($data_detail);
			...
			add_stock($r_purchases_details['item_id'], $r_purchases_details['item_type'],$i_supplier,
					...
					$supplier_id = $r_purchases['supplier_id'];
					$kategori_id = select_config_by('items', 'kategori_id', $where_item_id);

					$where_kategori_id = "WHERE kategori_id = '$kategori_id'";
					$kategori_keterangan_id = select_config_by('kategori_keterangan', 'kategori_keterangan_id', $where_kategori_id);
					$q_kategori_field = select_config('kategori_keterangan', $where_kategori_id);
					$id_kat = 0;
			}
			$where_purchase_id = "WHERE purchase_id = '$purchase_id'";
			$q_item_keterangan_details_tmp = select_config('item_keterangan_details_tmp', $where_purchase_id);
			while ($r_item_keterangan_details_tmp = mysql_fetch_array($q_item_keterangan_details_tmp)) {
				$data_keterangan = "'',
								 '$purchase_id',
								 '$item_id',
								 '$kategori_id',
								 '".$r_item_keterangan_details_tmp['kategori_keterangan_id']."',
								 '".$r_item_keterangan_details_tmp['keterangan_details']."',
								 '$supplier_id',
								 ''";
			...
			$where_purchase_id_ =  "purchase_id = '$purchase_id'";
			delete_config('item_keterangan_details_tmp',$where_purchase_id_);
			delete_item_tmp($purchase_id);
			delete_purchases_details_tmp($purchase_id);
			delete_purchases_tmp($purchase_id);
			if($i_payment_method == 5){
				create_journal(time(), "purchases.php?page=save_payment(Belum lunas)", 2, $i_payment_method, $uang_muka,
		...
		case 'menu_sub_2';
			...
			break;
			    ...
			    $item_name = $r_item_name['item_name'];
					...
					$where_item_id ="WHERE item_id = '$item_id'";
					$kategori = select_config_by('items', 'kategori_id',$where_item_id);

					$where_kategori_id ="WHERE kategori_id = '$kategori'";
					$q_kategori = select_config('kategori_keterangan', $where_kategori_id);
				...
				case 'keterangan_popmodal':
					...
					$item_qty = select_config_by('item_tmp', 'item_stock_qty', $where_item_id);
					$where_kategori_id = "WHERE kategori_id = '$kategori_id'";
					$q_kategori_keterangan = select_config('kategori_keterangan', $where_kategori_id);
					$action = "purchase.php?page=save_keterangan";
					include '../views/purchase/keterangan_popmodal.php';
					break;
				...
				case 'save_keterangan':
					...
					for ($i=0; $i < $baris; $i++) {
						$data_keterangan = "'',
																...
																'',
																''";
						create_config('item_keterangan_details_tmp',$data_keterangan);
						$on++;
						if ($on>=$no) {
							$on=0;
						}
				...
				case 'delete_widget':
					...
					delete_config("item_keterangan_details_tmp", "item_id = '$id'");
					$_SESSION['tanggal'] = $tanggal;
