...
switch ($page) {
	...
	case 'form':
		...
		$stock_buy = read_stock_buy($id, $get_new_date,$s_cabang);
		$unit_id_new_buy = get_unit_id_new_buy($id,$s_cabang);
	...
	case 'konversi_list':
		$id = $_POST['x'];
		$item_id = $_POST['y'];
		$query = select_unit_konversi($id,$item_id);
		while ($row = mysql_fetch_array($query)) {
			$data[] = array('item_id' 					=> $row['item_id'],
											'unit_id' 					=> $row['unit_id'],
		 									'unit_jml' 					=> $row['unit_jml'],
											'unit_konversi' 		=> $row['unit_konversi'],
											'unit_konversi_id' 		=> $row['unit_konversi_id'],
											'unit_konversi_jml' => $row['unit_konversi_jml'],
											'unit_utama'				=> $row['unit_utama'],
											'harga_konversi'		=> $row['harga_konversi'],
											'unit_konversi_name'=> $row['unit_konversi_name']
										);
		};
		echo json_encode($data);
		break;
	...
	case 'save_unit_konversi':
		...
		$i_harga_konversi = $_POST['i_harga_konversi'];
			$data = "'',
							...
							'$i_harga_konversi'
							";
			create_config("unit_konversi",$data);
		header("Location: stock.php?page=form&id=$item_id");
		break;
