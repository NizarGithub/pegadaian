...
function select_unit_konversi($id,$item_id){
	$query = mysql_query("SELECT a.*, b.unit_name AS unit_utama, c.unit_name AS unit_konversi_name  FROM unit_konversi a
												LEFT JOIN units b ON b.unit_id = a.unit_id
												LEFT JOIN units c ON c.unit_id = a.unit_konversi
												WHERE a.unit_id = '$id' and a.item_id = '$item_id'");
	return $query;
}
...
function read_stock_buy($id, $date, $branch_id){
	$query = mysql_query("SELECT a.*, d.* FROM item_stocks a
												JOIN purchases_details d on d.item_id = a.item_id
												where d.purchase_date = '$date' and branch_id = '$branch_id'
												and a.item_id = '$id'");
	$read_stock_buy = mysql_fetch_array($query);
	$purchase_price = ($read_stock_buy['purchase_price']) ? $read_stock_buy['purchase_price'] : 0;
