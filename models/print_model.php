...
function select_transaction_details_item($id){
	$query = mysql_query("SELECT a.*, b.*, c.unit_name FROM transaction_details a
												LEFT JOIN items b on b.item_id = a.item_id
												LEFT JOIN units c on c.unit_id = a.transaction_detail_unit
...
function total_yang_sudah_dibayar($id){
	$query = mysql_query("SELECT SUM(c.angsuran_nominal) as result FROM kredit a
