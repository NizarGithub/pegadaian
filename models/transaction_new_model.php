...
function select_keterangan_details($id, $kategori_keterangan_id){
	$query = mysql_query("SELECT a.* FROM item_keterangan_details a
												WHERE a.item_id = '$id' AND a.kategori_keterangan_id = '$kategori_keterangan_id'
												AND a.item_keterangan_details_id NOT IN ( SELECT keterangan_item FROM widget_tmp_details
		 										WHERE item_id = '$id' )
												AND a.status = 0");
  return $query;
}
