				...
				<center>
					...
					<div  id="frame" class="" style="width:80%;">
						<div class="row">
							...
							style="width: 80%;"  value="<?= format_rupiah($qty-1)?>" readonly>
...
<script>
	...
	$("#idForm").submit(function(e) {
		...
		$.ajax({
					 ...
					 {popmodal_keterangan(data.item_id, data.wt_id);}
				 });
		e.preventDefault();
	});

	function popmodal_keterangan(item_id, wt_id){
		var transaction_id = $('#transaction_id').val();
		$('#item_popmodal_details').modal({
			backdrop: 'static',
 			keyboard: false
		});
		var url = 'transaction_new.php?page=keterangan_popmodal&transaction_id='+transaction_id+'&item_id='+item_id+'&wt_id='+wt_id;
			$('#item_popmodal_details_content').load(url,function(result){
		});
	}
